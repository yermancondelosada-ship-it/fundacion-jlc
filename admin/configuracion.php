...
try {
    ...
    // Reparar tabla propuestas_valor (Asegurar columna icono e imagen)
    try {
    ...
    try {
        $db->query("ALTER TABLE propuestas_valor ADD COLUMN imagen VARCHAR(255) AFTER icono");
    } catch (Exception $e) { }

    $db->query("CREATE TABLE IF NOT EXISTS aliados (id INT AUTO_INCREMENT PRIMARY KEY, nombre VARCHAR(255) NOT NULL, logo VARCHAR(255) NOT NULL, orden INT DEFAULT 0)");
} catch (Exception $e) {
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['update_general'])) {
        foreach ($_POST as $key => $value) {
            if (in_array($key, ['update_general', 'update_carrusel_speed']))
                continue;
    ...
    if (isset($_POST['action_propuesta'])) {
        ...
        $imagen = '';
        if ($id) {
            $stmt = $db->prepare("SELECT imagen FROM propuestas_valor WHERE id = ?");
            $stmt->execute([$id]);
            $current = $stmt->fetch();
        ...
        if ($id) {
            $stmt = $db->prepare("UPDATE propuestas_valor SET titulo = ?, descripcion = ?, icono = ?, imagen = ? WHERE id = ?");
            $stmt->execute([$titulo, $descripcion, $icono, $imagen, $id]);
        } else {
            $stmt = $db->prepare("INSERT INTO propuestas_valor (titulo, descripcion, icono, imagen) VALUES (?, ?, ?, ?)");
            $stmt->execute([$titulo, $descripcion, $icono, $imagen]);
        }
    }
    ...
    if (isset($_POST['action_servicio'])) {
        ...
        // Mantener icono existente si es una actualización
        $icono = '';
        if ($id) {
            $stmt = $db->prepare("SELECT icono FROM servicios WHERE id = ?");
            $stmt->execute([$id]);
            $current = $stmt->fetch();
            $icono = $current ? $current->icono : '';
        }

        // Si subieron un icono como texto (font-awesome)
        if (!empty($_POST['icono_texto'])) {
            $icono = $_POST['icono_texto'];
        }

        // Si subieron una imagen
        if (isset($_FILES['icono_img']) && $_FILES['icono_img']['error'] == 0) {
            $ext = pathinfo($_FILES['icono_img']['name'], PATHINFO_EXTENSION);
            $filename = "servicio_" . time() . "." . $ext;
            if (move_uploaded_file($_FILES['icono_img']['tmp_name'], "../uploads/img/" . $filename)) {
                $icono = $filename;
            }
        }

        if ($id) {
            $stmt = $db->prepare("UPDATE servicios SET titulo = ?, descripcion = ?, icono = ? WHERE id = ?");
            $stmt->execute([$titulo, $descripcion, $icono, $id]);
        } else {
            $stmt = $db->prepare("INSERT INTO servicios (titulo, descripcion, icono) VALUES (?, ?, ?)");
            $stmt->execute([$titulo, $descripcion, $icono]);
        }
        header("Location: configuracion.php?success=1#servicios");
        exit;
    }
    ...
    // 8. Pilares Management
    if (isset($_POST['update_pilares'])) {
        for ($i = 1; $i <= 4; $i++) {
            $keys = ["pilar_{$i}_icono", "pilar_{$i}_titulo", "pilar_{$i}_desc"];
            foreach ($keys as $key) {
                if (isset($_POST[$key])) {
                    $stmt = $db->prepare("INSERT INTO site_config (llave, valor, categoria) VALUES (?, ?, 'institucion') ON DUPLICATE KEY UPDATE valor = ?");
                    $stmt->execute([$key, $_POST[$key], $_POST[$key]]);
        ...
        header("Location: configuracion.php?success=1#pilares");
        exit;
    }

    // 9. Aliados Management
    if (isset($_POST['action_aliado'])) {
        $nombre = $_POST['nombre'] ?? '';
        $logo = '';
        if (isset($_FILES['logo']) && $_FILES['logo']['error'] == 0) {
            $ext = pathinfo($_FILES['logo']['name'], PATHINFO_EXTENSION);
            $logo = 'aliado_' . time() . '.' . $ext;
            move_uploaded_file($_FILES['logo']['tmp_name'], '../uploads/img/' . $logo);
        }
        $stmt = $db->prepare("INSERT INTO aliados (nombre, logo) VALUES (?, ?)");
        $stmt->execute([$nombre, $logo]);
        header("Location: configuracion.php?success=1#institucion");
        exit;
    }
...
$slides = $db->query("SELECT * FROM carrusel ORDER BY orden ASC, id DESC")->fetchAll();
...
$servicios = $db->query("SELECT * FROM servicios ORDER BY orden ASC, id DESC")->fetchAll();
$propuestas = $db->query("SELECT * FROM propuestas_valor ORDER BY id ASC")->fetchAll();
...
<div id="config-container">
    ...
    <div id="view-dashboard" class="view-module active">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <!-- 1. Gestión Carrusel -->
            <div onclick="showModule('carrusel')"
            ...
            <div onclick="showModule('identidad')"
                class="module-card bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-100 flex flex-col items-center text-center">
                <div class="w-16 h-16 icon-box rounded-2xl flex items-center justify-center text-2xl mb-6">
            ...
            <div onclick="showModule('institucion')"
                class="module-card bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-100 flex flex-col items-center text-center">
                <div class="w-16 h-16 icon-box rounded-2xl flex items-center justify-center text-2xl mb-6">
            ...
            <div onclick="showModule('propuesta')"
                class="module-card bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-100 flex flex-col items-center text-center">
                <div class="w-16 h-16 icon-box rounded-2xl flex items-center justify-center text-2xl mb-6">
            ...
            <div onclick="showModule('pilares')"
                class="module-card bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-100 flex flex-col items-center text-center">
                <div class="w-16 h-16 icon-box rounded-2xl flex items-center justify-center text-2xl mb-6">
            ...
            <!-- 7. Redes y Contacto -->
            <div onclick="showModule('contacto')"
                class="module-card bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-100 flex flex-col items-center text-center">
                <div class="w-16 h-16 icon-box rounded-2xl flex items-center justify-center text-2xl mb-6">
            ...
            <!-- 9. Navegación -->
            <div onclick="showModule('navegacion')"
                class="module-card bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-100 flex flex-col items-center text-center">
                <div class="w-16 h-16 icon-box rounded-2xl flex items-center justify-center text-2xl mb-6">
        ...
        <section class="bg-white rounded-[2.5rem] shadow-xl p-10">
            ...
            <div class="bg-gray-50 rounded-3xl p-6 mb-8 border border-gray-100">
                <form action="configuracion.php" method="POST" class="flex items-center space-x-4">
                    ...
                    <input type="number" name="velocidad" value="<?php echo $carrusel_speed; ?>"
                        class="w-20 px-4 py-2 rounded-xl border-none shadow-inner">
                    <button type="submit"
                        class="bg-brand-700 text-white px-6 py-2 rounded-xl font-bold">ACTUALIZAR</button>
                </form>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <button onclick="openModal('slideModal')"
                    class="border-2 border-dashed border-gray-200 rounded-3xl p-10 flex flex-col items-center justify-center text-gray-400 hover:border-brand-500 hover:text-brand-500 transition-all">
                ...
                <?php foreach ($slides as $slide): ?>
                    <div class="relative group rounded-3xl overflow-hidden shadow-md">
                        ...
                        <div class="absolute inset-0 bg-black/40 p-4 flex flex-col justify-end text-white">
                            ...
                            <a href="?delete_slide=<?php echo $slide->id; ?>" onclick="return confirm('¿Borrar?')"
                                class="absolute top-2 right-2 bg-red-600 w-8 h-8 rounded-full flex items-center justify-center"><i
            ...
            <section class="bg-white rounded-[2.5rem] shadow-xl p-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div>
                        ...
                        <div
                            class="bg-gray-50 p-10 rounded-[2rem] border-2 border-dashed border-gray-200 text-center mb-4">
                            <img id="preview-logo" src="../uploads/img/<?php echo getConfig('logo'); ?>"
                                class="max-h-32 mx-auto mb-4">
                            <input type="file" name="logo" onchange="previewImage(this, 'preview-logo')" class="hidden"
                                id="logo-input">
                            <label for="logo-input"
                                class="cursor-pointer bg-white px-6 py-2 rounded-xl shadow-sm border border-gray-200 font-bold text-xs text-brand-700 hover:bg-brand-50 transition-all">CAMBIAR
                                LOGO</label>
                        </div>
                    </div>
                    <div>
                        ...
                        <div
                            class="bg-gray-50 p-10 rounded-[2rem] border-2 border-dashed border-gray-200 text-center mb-4">
                            <img id="preview-favicon" src="../uploads/img/<?php echo getConfig('favicon'); ?>"
                                class="w-16 h-16 mx-auto mb-4">
                            <input type="file" name="favicon" onchange="previewImage(this, 'preview-favicon')"
                                class="hidden" id="fav-input">
                            <label for="fav-input"
                                class="cursor-pointer bg-white px-6 py-2 rounded-xl shadow-sm border border-gray-200 font-bold text-xs text-brand-700 hover:bg-brand-50 transition-all">CAMBIAR
                                FAVICON</label>
                        </div>
                ...
                <div class="mt-10 pt-10 border-t">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="number" name="color_h" value="<?php echo getConfig('color_h', '142'); ?>"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" name="color_s" value="<?php echo getConfig('color_s', '70'); ?>"
                                class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-none font-bold">
                        </div>
                        <div class="space-y-2">
    ...
    <div id="module-institucion" class="view-module">
        <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            <button type="button" onclick="showDashboard()" class="text-brand-700 font-bold flex items-center"><i
                    class="fas fa-arrow-left mr-2"></i> Volver</button>
            <div class="flex space-x-2">
                <a href="#section-esencia"
                    class="px-4 py-2 bg-gray-100 rounded-xl text-xs font-bold text-gray-600 hover:bg-brand-50 hover:text-brand-700 transition-all">ESENCIA</a>
                <a href="#section-equipo"
        ...
        <section class="space-y-12">
            ...
            <form action="configuracion.php" id="form-institucion" method="POST"
                class="bg-white rounded-[2.5rem] shadow-xl p-10 space-y-10">
                <input type="hidden" name="update_general" value="1">
                ...
                <div class="grid grid-cols-1 gap-8">
                    <div>
                        ...
                        <input type="hidden" name="mision" id="input-mision"
                            value="<?php echo htmlspecialchars(getConfig('mision')); ?>">
                    </div>
                    <div>
                        ...
                        <input type="hidden" name="vision" id="input-vision"
                            value="<?php echo htmlspecialchars(getConfig('vision')); ?>">
                    </div>
                    <div>
                        ...
                        <input type="hidden" name="historia" id="input-historia"
                            value="<?php echo htmlspecialchars(getConfig('historia')); ?>">
                    </div>
                </div>
                <button type="submit"
                    class="bg-brand-700 text-white px-10 py-4 rounded-2xl font-black shadow-lg hover:bg-brand-800 transition-all w-full">
                    <i class="fas fa-save mr-2"></i> GUARDAR CONTENIDO PRINCIPAL
                </button>
            </form>

            <!-- 2. Nuestra Esencia (Valores y Filosofía) -->
            <form action="configuracion.php" method="POST" enctype="multipart/form-data" id="section-esencia"
                class="bg-white rounded-[2.5rem] shadow-xl p-10 space-y-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                    ...
                    <div class="space-y-6">
                        <h5 class="font-bold text-brand-700 flex items-center">
                            <span
                                class="w-8 h-8 bg-brand-100 text-brand-700 rounded-lg flex items-center justify-center mr-2 text-xs">1</span>
                            Nuestros Valores
                        </h5>
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Título</label>
                            <input type="text" name="valores_titulo"
                                value="<?php echo htmlspecialchars(getConfig('valores_titulo', 'Nuestros Valores')); ?>"
                                class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none font-bold">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Descripción</label>
                            <textarea name="valores_desc" rows="4"
                                class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none text-sm"><?php echo htmlspecialchars(getConfig('valores_desc')); ?></textarea>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-4">Imagen o Icono
                                acompañante</label>
                            <div class="flex items-center space-x-6 bg-gray-50 p-6 rounded-3xl">
                                <img src="../uploads/institucion/<?php echo getConfig('valores_img'); ?>"
                                    class="w-20 h-20 object-cover rounded-2xl bg-white shadow-sm"
                                    onerror="this.src='https://via.placeholder.com/150?text=Icono'">
                    ...
                    <div class="space-y-6">
                        <h5 class="font-bold text-yellow-600 flex items-center">
                            <span
                                class="w-8 h-8 bg-yellow-100 text-yellow-600 rounded-lg flex items-center justify-center mr-2 text-xs">2</span>
                            Nuestra Filosofía
                        </h5>
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Título</label>
                            <input type="text" name="filosofia_titulo"
                                value="<?php echo htmlspecialchars(getConfig('filosofia_titulo', 'Nuestra Filosofía')); ?>"
                                class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none font-bold">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Descripción</label>
                            <textarea name="filosofia_desc" rows="4"
                                class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none text-sm"><?php echo htmlspecialchars(getConfig('filosofia_desc')); ?></textarea>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase mb-4">Imagen o Icono
                                acompañante</label>
                            <div class="flex items-center space-x-6 bg-gray-50 p-6 rounded-3xl">
                                <img src="../uploads/institucion/<?php echo getConfig('filosofia_img'); ?>"
                                    class="w-20 h-20 object-cover rounded-2xl bg-white shadow-sm"
                                    onerror="this.src='https://via.placeholder.com/150?text=Icono'">
                ...
                <button type="submit"
                    class="bg-gray-900 text-white px-10 py-4 rounded-2xl font-black shadow-lg hover:bg-black transition-all w-full">
            ...
            <div id="section-equipo" class="bg-white rounded-[2.5rem] shadow-xl p-10">
                <div class="flex justify-between items-center mb-10 border-b pb-6">
                    ...
                    <button onclick="openModal('equipoModal'); resetEquipoForm();"
                        class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-2xl font-bold flex items-center shadow-md transition-all text-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <?php foreach ($equipo as $person): ?>
                        <div
                            class="bg-gray-50 rounded-3xl p-6 border border-gray-100 flex items-center space-x-4 group relative">
                            <img src="../uploads/institucion/<?php echo $person->foto; ?>"
                                class="w-16 h-16 rounded-full object-cover border-4 border-white shadow-md"
                                onerror="this.src='https://ui-avatars.com/api/?name=<?php echo urlencode($person->nombre); ?>&background=random'">
                            <div class="flex-grow min-w-0">
                                <h5 class="font-bold text-gray-900 truncate"><?php echo $person->nombre; ?></h5>
                                <p class="text-brand-600 text-xs font-bold uppercase tracking-wider">
                                    <?php echo $person->cargo; ?></p>
                            </div>
                            <div class="flex flex-col space-y-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onclick='editEquipo(<?php echo json_encode($person); ?>)'
                                    class="text-blue-500 hover:text-blue-700"><i class="fas fa-edit"></i></button>
                                <a href="?delete_equipo=<?php echo $person->id; ?>"
                                    onclick="return confirm('¿Eliminar integrante?')"
                                    class="text-red-400 hover:text-red-600"><i class="fas fa-trash"></i></a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                    <?php if (empty($equipo)): ?>
                        <div class="col-span-full py-10 text-center text-gray-400 italic">No hay integrantes registrados.
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div id="section-aliados" class="bg-white rounded-[2.5rem] shadow-xl p-10 mt-10">
                <div class="flex justify-between items-center mb-10 border-b pb-6">
                    <div>
                        <h4 class="text-2xl font-bold">Aliados Estratégicos</h4>
                        <p class="text-gray-500 text-sm mt-1">Sube los logos de las instituciones u organizaciones aliadas.</p>
                    </div>
                    <button onclick="openModal('aliadoModal')"
                        class="bg-brand-600 hover:bg-brand-700 text-white px-6 py-3 rounded-2xl font-bold flex items-center shadow-md transition-all text-sm">
                        <i class="fas fa-plus mr-2"></i> + AÑADIR ALIADO
                    </button>
                </div>

                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                    <?php foreach ($aliados as $aliado): ?>
                        <div class="bg-gray-50 rounded-3xl p-4 border border-gray-100 flex flex-col items-center group relative text-center">
                            <img src="../uploads/img/<?php echo $aliado->logo; ?>" class="h-16 w-auto object-contain mb-2" alt="<?php echo $aliado->nombre; ?>">
                            <span class="text-xs font-bold text-gray-500 truncate w-full"><?php echo $aliado->nombre; ?></span>
                            <a href="?delete_aliado=<?php echo $aliado->id; ?>"
                                onclick="return confirm('¿Eliminar aliado?')"
                                class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs transition-opacity"><i class="fas fa-times"></i></a>
                        </div>
                    <?php endforeach; ?>
                    <?php if (empty($aliados)): ?>
                        <div class="col-span-full py-10 text-center text-gray-400 italic">No hay aliados registrados.</div>
                    <?php endif; ?>
                </div>
            </div>
        </section>
        ...
        <form action="configuracion.php" method="POST" enctype="multipart/form-data">
            ...
            <section class="bg-white rounded-[2.5rem] shadow-xl p-10">
                ...
                <div class="bg-brand-50 p-8 rounded-3xl mb-10 border border-brand-100">
                    ...
                    <select name="show_social_section"
                        class="w-full bg-white border-none rounded-2xl font-bold text-brand-700 px-6 py-4 shadow-sm">
                        <option value="1" <?php echo getConfig('show_social_section') == '1' ? 'selected' : ''; ?>>
                            ACTIVADA EN INICIO</option>
                        <option value="0" <?php echo getConfig('show_social_section') == '0' ? 'selected' : ''; ?>>
                            DESACTIVADA EN INICIO</option>
                    </select>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="contact_phone" value="<?php echo getConfig('contact_phone'); ?>"
                            class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-none font-bold">
                    </div>
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-gray-500">EMAIL OFICIAL</label>
                        <input type="email" name="contact_email" value="<?php echo getConfig('contact_email'); ?>"
                            class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-none font-bold">
                    </div>
                    <div class="md:col-span-2 space-y-2">
                        <label class="text-xs font-bold text-gray-500">DIRECCIÓN FÍSICA</label>
                        <input type="text" name="contact_address" value="<?php echo getConfig('contact_address'); ?>"
                            class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-none font-bold">
                    </div>
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-gray-500">FACEBOOK URL</label>
                        <input type="text" name="social_fb" value="<?php echo getConfig('social_fb'); ?>"
                            class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-none">
                    </div>
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-gray-500">INSTAGRAM URL</label>
                        <input type="text" name="social_ig" value="<?php echo getConfig('social_ig'); ?>"
                            class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-none">
                    </div>
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-gray-500">NÚMERO DE WHATSAPP</label>
                        <input type="text" name="social_wa" value="<?php echo getConfig('social_wa'); ?>"
                            placeholder="Ej: 573001234567"
                            class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-none font-bold text-green-600">
                    </div>
                </div>
            </section>
            <button type="submit"
                ...
                <div class="bg-gray-50 p-8 rounded-3xl border">
                    ...
                    <form action="configuracion.php" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="nombre_tema" placeholder="Ej: Navidad, Aniversario"
                                class="w-full px-4 py-3 rounded-xl border-none shadow-sm">
                        </div>
                        ...
                        <button type="submit"
                            class="w-full bg-brand-700 text-white font-bold py-4 rounded-xl shadow-lg mt-4">AGREGAR AL
                            CALENDARIO</button>
                    </form>
                </div>
                <div>
    ...
    <div id="module-propuesta" class="view-module">
        ...
        <section class="bg-white rounded-[2.5rem] shadow-xl p-10">
            <div class="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
                ...
                <button onclick="openModal('propuestaModal'); resetPropuestaForm();"
                    class="bg-brand-700 hover:bg-brand-800 text-white px-8 py-4 rounded-2xl font-bold flex items-center shadow-lg transition-all">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <?php foreach ($propuestas as $prop): ?>
                    <div
                        class="bg-white rounded-[2rem] border border-gray-100 p-8 shadow-md hover:shadow-xl transition-all group relative">
                        ...
                        <div class="flex items-center space-x-3 pt-6 border-t border-gray-50">
                            <button onclick='editPropuesta(<?php echo json_encode($prop); ?>)'
                                class="flex-grow bg-brand-50 text-brand-700 font-bold py-3 rounded-xl hover:bg-brand-700 hover:text-white transition-all text-xs flex items-center justify-center">
                                <i class="fas fa-pencil-alt mr-2"></i> EDITAR
                            </button>
                            <a href="?delete_propuesta=<?php echo $prop->id; ?>"
                                onclick="return confirm('¿Seguro que deseas eliminar esta propuesta?')"
                                class="w-12 h-12 bg-red-50 text-red-500 rounded-xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-all">
        ...
        <section class="bg-white rounded-[2.5rem] shadow-xl p-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <?php foreach ($servicios as $srv): ?>
                    <div
                        class="p-6 bg-gray-50 rounded-3xl border flex items-center justify-between group hover:border-brand-500 transition-all">
                        <div class="flex items-center space-x-4">
                            <div
                                class="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-brand-700 shadow-sm">
                                <i class="fas fa-check"></i>
                            </div>
                            <span class="font-bold text-gray-800"><?php echo $srv->titulo; ?></span>
                        </div>
                        <a href="?delete_service=<?php echo $srv->id; ?>" class="text-red-400 hover:text-red-600"
                            onclick="return confirm('¿Eliminar?')"><i class="fas fa-trash"></i></a>
                    </div>
                <?php endforeach; ?>
            </div>
        </section>
    ...
    <div id="module-pilares" class="view-module">
        ...
        <section class="bg-white rounded-[2.5rem] shadow-xl p-10">
            ...
            <form action="configuracion.php" method="POST">
                <input type="hidden" name="update_pilares" value="1">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <?php
                    ...
                    for ($i = 1; $i <= 4; $i++):
                        ...
                        <div class="bg-gray-50 p-8 rounded-3xl border space-y-4">
                            <h5 class="font-bold text-brand-700">Pilar <?php echo $i; ?></h5>
                            <input type="text" name="pilar_<?php echo $i; ?>_icono"
                                value="<?php echo getConfig("pilar_{$i}_icono", $default_icons[$i - 1]); ?>"
                                class="w-full px-4 py-3 rounded-xl border-none shadow-sm font-bold text-sm text-gray-400"
                                placeholder="Icono (ej: fas fa-star)">
                            <input type="text" name="pilar_<?php echo $i; ?>_titulo"
                                value="<?php echo getConfig("pilar_{$i}_titulo", $default_titles[$i - 1]); ?>"
                                class="w-full px-4 py-3 rounded-xl border-none shadow-sm font-bold">
                            <textarea name="pilar_<?php echo $i; ?>_desc"
                                class="w-full px-4 py-3 rounded-xl border-none shadow-sm text-sm"
                                rows="3"><?php echo getConfig("pilar_{$i}_desc", "Descripción del pilar $i"); ?></textarea>
                        </div>
                    <?php endfor; ?>
                </div>
                <button type="submit"
    ...
    <div id="module-navegacion" class="view-module">
        ...
        <section class="bg-white rounded-[2.5rem] shadow-xl p-10">
            <div class="flex justify-between items-center mb-8">
                ...
                <button type="button" onclick="addNavRow()"
                    class="bg-brand-100 text-brand-700 hover:bg-brand-200 px-6 py-2 rounded-xl font-bold transition-all text-sm flex items-center">
            ...
            <form action="configuracion.php" method="POST" class="space-y-6">
                ...
                <div class="bg-gray-50 rounded-3xl p-6 border border-gray-100">
                    <div
                        class="grid grid-cols-12 gap-4 mb-4 text-xs font-bold text-gray-500 uppercase tracking-widest px-4">
                    ...
                    <div id="nav-links-container" class="space-y-3">
                        <?php foreach ($navbar_links as $index => $link): ?>
                            <div
                                class="nav-link-row grid grid-cols-12 gap-4 items-center bg-white p-3 rounded-2xl shadow-sm border border-gray-100">
                                <div class="col-span-5">
                                    <input type="text" name="navbar_text[]"
                                        value="<?php echo htmlspecialchars($link['text']); ?>" required
                                        class="w-full px-4 py-3 bg-gray-50 rounded-xl border-none font-bold text-sm"
                                        placeholder="Ej: INICIO">
                                </div>
                                <div class="col-span-6">
                                    <input type="text" name="navbar_url[]"
                                        value="<?php echo htmlspecialchars($link['url']); ?>" required
                                        class="w-full px-4 py-3 bg-gray-50 rounded-xl border-none text-sm"
                                        placeholder="Ej: index.php">
                                </div>
                                <div class="col-span-1 flex justify-center">
                                    <button type="button" onclick="this.closest('.nav-link-row').remove()"
                                        class="w-10 h-10 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-xl flex items-center justify-center transition-all">
...
<!-- MODAL: SERVICIO -->
<div id="serviceModal"
    class="fixed inset-0 z-50 hidden bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm">
    <div class="bg-white rounded-[3rem] shadow-2xl max-w-2xl w-full p-10 overflow-hidden">
        <div class="flex items-center justify-between mb-8">
            <h3 class="text-2xl font-bold text-gray-900">Nuevo Servicio</h3>
            <button onclick="closeModal('serviceModal')" class="text-gray-400 hover:text-gray-600"><i
                    class="fas fa-times text-xl"></i></button>
        </div>

        <form action="configuracion.php" method="POST" enctype="multipart/form-data" class="space-y-6">
            <input type="hidden" name="action_servicio" value="1">
            <input type="hidden" name="id" id="srv-id">

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Título del Servicio</label>
                    <input type="text" name="titulo" id="srv-titulo" required
                        class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none font-bold">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Descripción</label>
                    <textarea name="descripcion" id="srv-descripcion" rows="4" required
                        class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none text-sm"></textarea>
                </div>

                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Icono (Ej: fas fa-star)</label>
                    <input type="text" name="icono_texto" id="srv-icono-texto"
                        class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none font-bold">
                </div>
                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">O subir Imagen Icono</label>
                    <input type="file" name="icono_img" class="text-xs mt-4">
                </div>
            </div>

            <div class="flex space-x-4 pt-6 border-t border-gray-100">
                <button type="submit"
                    class="flex-grow bg-brand-600 text-white font-bold py-4 rounded-2xl shadow-xl hover:bg-brand-700 transition-all uppercase tracking-widest">
                    GUARDAR SERVICIO
                </button>
                <button type="button" onclick="closeModal('serviceModal')"
                    class="px-8 py-4 text-gray-500 font-bold hover:bg-gray-50 rounded-2xl transition-all">CANCELAR</button>
            </div>
        </form>
    </div>
</div>
...
<!-- MODAL: PROPUESTA -->
<div id="propuestaModal"
    class="fixed inset-0 z-50 hidden bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm">
    <div class="bg-white rounded-[3rem] shadow-2xl max-w-2xl w-full p-10 overflow-hidden">
        <div class="flex items-center justify-between mb-8">
            <h3 class="text-2xl font-bold text-gray-900" id="propuesta-modal-title">Nueva Propuesta de Valor</h3>
            <button onclick="closeModal('propuestaModal')" class="text-gray-400 hover:text-gray-600"><i
                    class="fas fa-times text-xl"></i></button>
        </div>
        ...
        <form action="configuracion.php" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Título de la
                        Propuesta</label>
                    <input type="text" name="titulo" id="prop-titulo" required placeholder="Ej: Educación Disruptiva"
                        class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none focus:ring-2 focus:ring-brand-500 font-bold">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Descripción
                        Detallada</label>
                    <textarea name="descripcion" id="prop-descripcion" rows="4" required
                        placeholder="Explica brevemente de qué trata esta propuesta..."
                        class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none focus:ring-2 focus:ring-brand-500 text-sm"></textarea>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Selecciona un
                        Icono Visual</label>
                    ...
                    <div class="grid grid-cols-4 sm:grid-cols-8 gap-4 bg-gray-50 p-6 rounded-[2rem]">
                        ...
                        foreach ($common_icons as $class => $name): ?>
                            <div onclick="selectPropuestaIcon('<?php echo $class; ?>', this)"
                                class="prop-icon-option w-12 h-12 rounded-xl flex items-center justify-center text-xl cursor-pointer hover:bg-brand-100 hover:text-brand-700 transition-all <?php echo $class == 'fas fa-star' ? 'bg-brand-500 text-white shadow-lg' : 'bg-white text-gray-400 shadow-sm'; ?>">
                ...
                <div class="md:col-span-2">
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Opcional: Imagen Diferencial (16:9)</label>
                    <input type="file" name="imagen" class="text-xs">
                </div>
            </div>
                ...
                <div class="md:col-span-2 flex items-center justify-between bg-brand-50 p-6 rounded-2xl">
                    ...
                    <div
                        class="w-12 h-12 bg-white text-brand-700 rounded-xl flex items-center justify-center text-2xl shadow-sm">
            ...
            <div class="flex space-x-4 pt-8">
                <button type="submit"
                    class="flex-grow bg-brand-700 text-white font-black py-5 rounded-2xl shadow-xl hover:bg-brand-800 transition-all uppercase tracking-widest">
                    GUARDAR PROPUESTA
                </button>
                <button type="button" onclick="closeModal('propuestaModal')"
                    class="px-10 py-5 text-gray-400 font-bold hover:text-gray-600 transition-all">
                    CANCELAR
                </button>
            </div>
        </form>
    ...
    <div class="bg-white rounded-[3rem] shadow-2xl max-w-lg w-full p-10">
        ...
        <form action="configuracion.php" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="flex space-x-4 pt-6">
                <button type="submit"
                    class="flex-grow bg-brand-700 text-white font-bold py-4 rounded-2xl shadow-xl hover:bg-brand-800 transition-all">SUBIR
                    SLIDE</button>
                <button type="button" onclick="closeModal('slideModal')"
                    class="px-8 py-4 text-gray-500 font-bold">CANCELAR</button>
            </div>
        </form>
    </div>
</div>
...
<div id="equipoModal"
    ...
    <div class="bg-white rounded-[3rem] shadow-2xl max-w-xl w-full p-10">
        <div class="flex items-center justify-between mb-8">
            ...
            <button onclick="closeModal('equipoModal')" class="text-gray-400 hover:text-gray-600"><i
                    class="fas fa-times text-xl"></i></button>
        </div>
        ...
        <form action="configuracion.php" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nombre" id="equipo-nombre" required
                        class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none font-bold">
                </div>
                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase mb-2">Cargo / Título</label>
                    <input type="text" name="cargo" id="equipo-cargo" required
                        class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none font-bold">
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="frase" id="equipo-frase" rows="3"
                        class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none text-sm"></textarea>
                </div>
            </div>
            ...
            <div class="flex space-x-4 pt-6">
                <button type="submit"
                    class="flex-grow bg-green-600 text-white font-bold py-4 rounded-2xl shadow-xl hover:bg-green-700 transition-all uppercase">
                    GUARDAR PERFIL
                </button>
                <button type="button" onclick="closeModal('equipoModal')"
                    class="px-8 py-4 text-gray-500 font-bold">CANCELAR</button>
            </div>
        </form>
    </div>
...
<div id="aliadoModal"
    ...
    <div class="bg-white rounded-[3rem] shadow-2xl max-w-xl w-full p-10">
        <div class="flex items-center justify-between mb-8">
            <h3 class="text-2xl font-bold text-gray-900">Añadir Aliado</h3>
            <button onclick="closeModal('aliadoModal')" class="text-gray-400 hover:text-gray-600"><i
                    class="fas fa-times text-xl"></i></button>
        </div>

        <form action="configuracion.php" method="POST" enctype="multipart/form-data" class="space-y-6">
            <input type="hidden" name="action_aliado" value="1">
            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase mb-2">Nombre de la Organización</label>
                    <input type="text" name="nombre" required
                        class="w-full px-6 py-4 bg-gray-50 rounded-2xl border-none font-bold">
                </div>
                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase mb-2">Logo</label>
                    <input type="file" name="logo" required class="text-xs">
                </div>
            </div>

            <div class="flex space-x-4 pt-6">
                <button type="submit"
                    class="flex-grow bg-brand-600 text-white font-bold py-4 rounded-2xl shadow-xl hover:bg-brand-700 transition-all uppercase">
                    GUARDAR ALIADO
                </button>
                <button type="button" onclick="closeModal('aliadoModal')"
                    class="px-8 py-4 text-gray-500 font-bold">CANCELAR</button>
            </div>
        </form>
    </div>
